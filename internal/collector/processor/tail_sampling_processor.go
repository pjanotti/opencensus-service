...
type tailSamplingSpanProcessor struct {
	...
	maxNumTraces    uint64
	policies        []*Policy
	policiesCtx     []context.Context
	logger          *zap.Logger
	idToTrace       sync.Map
	policyTicker    tTicker
	decisionBatcher idbatcher.Batcher
	deleteChan      chan idbatcher.ID
	numTracesOnMap  uint64
	// numAddedTraces  uint64
}
...
func NewTailSamplingSpanProcessor(policies []*Policy, maxNumTraces uint64, decisionWait time.Duration, logger *zap.Logger) SpanProcessor {
	...
	inBatcher, _ := idbatcher.New(numDecisionBatches, 64, uint64(2*runtime.NumCPU())) // TODO: (@tail) constants or from config, same below // TODO: (@tail) check errors?
	...
	tsp.policiesCtx = make([]context.Context, 0, len(policies))
	for _, policy := range policies {
		mutator := tag.Insert(tagPolicyKey, policy.Name)
		policyCtx, _ := tag.New(tsp.ctx, mutator) // TODO: (@tail) check error
		tsp.policiesCtx = append(tsp.policiesCtx, policyCtx)
	}
	tsp.policyTicker = newPolicyTicker(tsp.samplingPolicyOnTick)
	tsp.deleteChan = make(chan idbatcher.ID, maxNumTraces)
	return tsp
}
...
func (tsp *tailSamplingSpanProcessor) samplingPolicyOnTick() {
	...
	startTime := time.Now()
	batch, _ := tsp.decisionBatcher.CloseCurrentAndTakeFirstBatch() // TODO: (@tail) special treatment when there is no more data
	batchLen := len(batch)
...
func (tsp *tailSamplingSpanProcessor) ProcessSpans(batch *agenttracepb.ExportTraceServiceRequest, spanFormat string) (uint64, error) {
	...
	for id, spans := range idToSpans {
		...
		if loaded {
			...
			traceID := []byte(id)
			tsp.decisionBatcher.AddToCurrentBatch(traceID)
			// tsp.deletionBatcher.AddToCurrentBatch(traceID)
			atomic.AddUint64(&tsp.numTracesOnMap, 1)
			postDeletion := false
			currTime := time.Now()
			for !postDeletion {
				select {
				case tsp.deleteChan <- traceID:
					postDeletion = true
				default:
					toDelete := <-tsp.deleteChan
					tsp.dropTrace(toDelete, currTime)
				}
			}
		}
...
func (tsp *tailSamplingSpanProcessor) dropTrace(traceID []byte, deletionTime time.Time) {
	var trace *sampling.TraceData
	key := traceKey(traceID)
	if d, ok := tsp.idToTrace.Load(key); ok {
		trace = d.(*sampling.TraceData)
		tsp.idToTrace.Delete(key)
	...
	for j := 0; j < policiesLen; j++ {
		if trace.Decision[j] == sampling.Pending {
			...
			if decision, err := policy.Evaluator.OnDroppedSpans(traceID, trace); err != nil {
